...
public class FrameworkFilter implements Filter {
	...
	public static void setUserToSession(HttpServletRequest request, HttpServletResponse response,String loginName) {
		setUserToSessionInternal(request, response, FrameworkConstants.DEFAULT_SKIN, loginName);
	}

	private static void setUserToSessionInternal(HttpServletRequest request, HttpServletResponse response, String skin, String loginName) {
		User user = FrameworkFactory.getAuthorizationManager().getUserPermissionByLoginName(loginName, ProjectConfig.PROJECT_NAME);
		if (user != null && IBSConstants.PLATEFORM_ADMIN == user.getPlatform() || IBSConstants.PLATEFORM_SUPPLIER == user.getPlatform()) {
			HttpSession session = request.getSession();
